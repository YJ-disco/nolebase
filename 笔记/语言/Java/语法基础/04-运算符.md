...
因为数字在程序中可以分为两种，一种是整型，一种是浮点型（[[03-数据类型与包装类|数据类型与包装类]]），整型和整型的运算结果就是整型，不会出现浮点型。否则，就会出现浮点型。
...
在学习位运算符之前，需要先学习一下二进制，因为位运算符操作的是整型数值（int、long、short、char、byte）对应的二进制，而不是数值本身。

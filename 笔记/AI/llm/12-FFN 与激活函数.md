...
算力上的差别相对 GEMM 可以忽略 —— 这正是「激活函数选型主要看效果、不看速度」的原因。
```
...
总参数量靠把 d_ff 从 4d 收到 8/3 d 维持不变。
```
...
All-to-All 的通信量随 token 数与专家分布变化，是 MoE 训练的主要开销来源。
```

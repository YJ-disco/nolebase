...
```
X:                     (1, 6, 512)
Q/K/V = X @ W^T:       (1, 6, 512) @ (512, 512) → (1, 6, 512)
view(1,6,8,64):        (1, 6, 8, 64)
transpose(1,2):        (1, 8, 6, 64)     ← head 维提到前面，8 个头独立算
K.transpose(-2,-1):    (1, 8, 64, 6)
Q @ K^T:               (1,8,6,64) @ (1,8,64,6) → (1, 8, 6, 6)
/ sqrt(64):            (1, 8, 6, 6)       ← sqrt(64)=8
softmax(dim=-1):       (1, 8, 6, 6)       ← 每行 6 个元素和为 1
attn @ V:              (1,8,6,6) @ (1,8,6,64) → (1, 8, 6, 64)
transpose(1,2):        (1, 6, 8, 64)
view(1,6,512):         (1, 6, 512)        ← 8 × 64 拼回 d_model
context @ W_O^T:       (1, 6, 512)
```
...
## 五、Softmax 的数值稳定性与 Online Softmax

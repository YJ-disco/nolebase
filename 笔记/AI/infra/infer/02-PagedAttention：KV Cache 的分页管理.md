...
块表把「逻辑块」映射到「物理块」（`block_size = 4` 示意，一个请求 Prefill 了 9 个 Token ⇒ 3 个逻辑块）：

```
  块表（每个请求一张）               物理显存里的块池
  ┌───────────┬───────────┐        ┌────┬────┬────┬────┬────┬────┬────┐
  │ 逻辑块 0  │ 物理块 #7 │───────▶│ #0 │ #1 │ #2 │ #3 │ #4 │ #5 │ #6 │
  ├───────────┼───────────┤        └────┴────┴────┴────┴────┴────┴────┘
  │ 逻辑块 1  │ 物理块 #2 │──┐          ▲                        ▲   ▲
  ├───────────┼───────────┤  └──────────┼────────────────────────┘   │
  │ 逻辑块 2  │ 物理块 #5 │─────────────┼────────────────────────────┘
  ...
  请求看到的是连续的逻辑块序列，物理上散落在任意位置
  ⇒ 填满一块才申请下一块：用多少申请多少，绝不提前预留

  （注意：这里的 KV Block 是 vLLM 自己的显存管理单位，
    与 CUDA 的 thread block 完全是两回事）
  ...
  连续分配与分页在碎片上的差别：
    ...
    连续分配（按最大长度预留）
      ┌────────────────────────────────────────┐ ┌──────┐┌────────┐
      │ 请求 A：预留 2048 格，实际只用了 100 格  │ │ 空隙 ││ 请求 B │
      └────────────────────────────────────────┘ └──────┘└────────┘
        └──── 内部碎片：剩下 1948 格被锁着没用 ──┘   └─ 外部碎片 ─┘
      实测利用率 20.4% – 38.2%（浪费 60% – 80%）

    分页（固定大小的块 + 按需分配）
      ┌──┬──┬──┬──┬┬──┬──┬──┬──┬┬──┐
      │▣ │▣ │▣ │▣ ││▣ │▣ │▣ │▣ ││▣ │
      └──┴──┴──┴──┴┴──┴──┴──┴──┴┴──┘
        ├─ 块大小固定 ⇒ 任何空闲块都能被任何请求用（外部碎片消除）
        └─ 最多浪费不到 1 个块（block_size = 16 时最多 15 个 Token 的位置）
      实测利用率 > 96%（浪费 < 4%）
```

## Kernel 视角：分散的块怎么被读出来
...
```
  两个请求共享同一个物理块（引用计数 ref_cnt = 2）

    请求 A 的块表 ──┐
                    ├──▶ 物理块 #7（ref_cnt = 2）
    请求 B 的块表 ──┘

  A 要往这个块里写新内容 ⇒ 写时复制（CoW）
    ① 先把 #7 复制成 #8
    ② A 的块表改指向 #8，在副本上写
    ③ #7 的 ref_cnt 降为 1，B 完全不受影响

  ⇒ 与 Linux fork() 之后父子进程共享内存页是同一个机制
  ⇒ 也是前缀缓存的物理基础（把共享从「并存请求」延伸到「先后请求」）

  ──────────────────────────────────────────────────────────────

  物理块池被抽干时的抢占（保证不 OOM、请求不失败的兜底）：

    换出（Swapping）        KV 拷到 CPU 内存，恢复时拷回
                            ⇒ 不用重算，但占 CPU 内存、有 PCIe 开销
    重计算（Recomputation） 直接丢弃 KV，恢复时把已生成的 Token 当新 Prompt 重新 Prefill
                            ⇒ 不占 CPU 内存、实现简单，但要重跑一次 Prefill

    该文口径：重计算的开销从不超过换出的 20%；块大小 ≤ 64 Token 时重算通常比换出更快
    vLLM V1 的默认是重计算
```

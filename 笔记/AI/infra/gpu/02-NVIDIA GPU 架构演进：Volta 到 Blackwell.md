...
五代架构的时间线（每一代回应上一代暴露的瓶颈）：

```
  2017 ── Volta（V100）      首次 Tensor Core：FP16 计算 + FP32 累加
    │                         ⇒ AMP 与 loss scaling 的硬件起点
  2018 ── Turing（T4）       INT8 / INT4 Tensor Core、RT Core，TDP 70W
    │                         ⇒ 量化在工业界铺开（TensorRT 的靶子）
  2020 ── Ampere（A100）     TF32、BF16、MIG，第三代 Tensor Core
    │                         ⇒ LLM 训练的默认精度切到 BF16
  2022 ── Hopper（H100）     FP8 + Transformer Engine、NVLink 4.0、TMA
    │                         ⇒ 回应 LLM 的算力饥渴与通信瓶颈
  2024 ── Blackwell（B200）  FP4、NVLink 5.0、双芯封装、累加器落 TMEM
                              ⇒ 单 die 算力追不上模型膨胀，转向封装与互联
```
...
算力与带宽的增长在拉开差距（以 V100 为 1）：

```
                  V100     A100     H100     B200
  FP16 算力        1×      2.5×     7.9×     18×
  显存带宽         1×      2.3×     3.7×     8.9×
  NVLink 带宽      1×       2×       3×       6×

        └──── 算力涨 18 倍，显存带宽只涨约 9 倍 ────┘

  ⇒ memory-bound 的特性逐代加重：算力买得到，喂饱算力的带宽跟不上。
```

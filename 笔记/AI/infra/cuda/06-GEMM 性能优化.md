...
从朴素实现到接近 cuBLAS 的八个台阶（每一级的本质都是提高算术强度）：

```
  Naive                      6% – 11%
  ＋Shared Memory Tiling  ──▶ 25% – 40%      复用从全局内存提到共享内存
  ＋Thread Tiling        ──▶ 50% – 65%   ▲ 最大的一级（+25pp）
  ＋Vectorized Load       ──▶ 60% – 75%      访存指令数降到 1/4
  ＋Bank Conflict 消除    ──▶ 70% – 80%
  ＋Warp Tiling          ──▶ 80% – 87%
  ＋Double Buffering     ──▶ 90% – 97%   ▲ 唯一的单级 10pp
  ＋SASS 手调             ──▶ 95% – 99%      前七级不做，这一级拿不到任何东西

  ⇒ 台阶的顺序本身就是知识：Tiling 之后瓶颈会转到共享内存、
    向量化会改变冲突的形态、不做双缓冲就永远卡在 87%。
```
...
```
 ...
 沿   │  Tile  │        │        │          B 的一段（BK×BN）
  M   ├────────┼────────┼────────┤
 方向 │        │        │        │        一次加载的数据被复用 BM×BN 次
      └────────┴────────┴────────┘
                        ▲
              沿 K 方向循环：每轮加载一块 A、一块 B，逐步累加
              （C = AB 的累加本来就沿 K 方向，所以切 K 是对的）

  ⇒ Tile 越大复用越高，但受共享内存容量限制；协作加载要求
    每个 Tile 维度能被线程排布整除，否则要加边界判断（本身就是分支）。
```
...
Thread Tiling 把复用再往上提一层（共享内存 → 寄存器）：

```
  从共享内存读入                    在寄存器里做外积
    A 的一段（TM 个）                 C[i][j] += A[i] × B[j]   （TM×TN 次乘加）
    B 的一段（TN 个）
      ...
      共享内存访问次数 ∝ 1/TM + 1/TN
      TM = TN = 8 ⇒ 共享内存访问量降到 1/4

  ⇒ 代价：64 个累加器 + 操作数 + 地址 ≈ 120–128 个寄存器/线程
         ⇒ Occupancy 只有 25%，但复用换来的效率远超这点损失
```
...
Double Buffering：让「加载下一块」与「计算当前块」重叠（两级都要做）

```
  K-Loop 级（全局内存 → 共享内存，藏 HBM 的 300–400 cycles）
  ...
  p-Loop 级（共享内存 → 寄存器，藏共享内存的 20–30 cycles）

      加载片段 p+1   ████████
      计算片段 p          ████████

  ⇒ 只做 K-Loop 级的话，瓶颈会从 HBM 转移到共享内存（12 TB/s 对 20 TB/s）
```

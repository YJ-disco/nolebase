...
> **TP 的通信频率最高**（每层都要），且通信量与激活大小相关 —— 这就是它必须待在 NVLink 范围内的原因，量化估算见 [[03-多卡互联与集群网络]]。
...
```
  中心化归约（所有 GPU 发给 rank 0，rank 0 求和后广播回去）

      GPU0 ◀──────── GPU1
       ▲   ╲
       │    ╲────── GPU2        rank 0 的入口/出口带宽成为瓶颈，
       │    ╱                  其余链路大部分时间空闲
       └───╱─────── GPU3
...
Ring AllReduce 的两阶段（4 个 rank，每份数据均分成 4 块）：

```
  ReduceScatter（N−1 = 3 轮）

      rank0 ──▶ rank1 ──▶ rank2 ──▶ rank3 ──┐
        ▲                                   │
        └───────────────────────────────────┘

      每轮把自己的块发给下一个、收上一个的块并就地累加；
      第 2、3 轮发的是"已经累加过的"那种块
      ⇒ 3 轮后每个 rank 手里恰好有一块完全归约的数据（占全部的 1/N）

  AllGather（再 3 轮）

      把手里那块已归约的数据沿环继续传出去
      ⇒ 再 3 轮后每个 rank 都拥有完整结果

  每 rank 通信量 = 2 × (N−1) × M/N —— N 大时趋近 2M，与节点数基本无关
```

## Tree AllReduce：低延迟的另一条路
...
通信藏在计算里（DDP 的 bucket 机制）：

```
  时间 ────────────────────────────────────────────────▶

  反向计算   [ 第 L 层 ][ 第 L−1 层 ][ 第 L−2 层 ][  …  ]
  通信          [AllReduce L][AllReduce L−1][AllReduce L−2]
                 └── 第 L 层梯度一算完就异步触发，不等整个模型

  ⇒ 通信与后续层的计算并行

  bucket_cap_mb 是双向权衡：
      太大 ⇒ 要攒够更多梯度才触发 ⇒ 等待时间长、重叠效果差
      太小 ⇒ 通信次数多 ⇒ 每次的固定启动开销占比上升
      默认 25 MB 是较好的起点
```

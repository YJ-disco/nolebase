...
```
   ┌──────────────────────────────┐
   │ agents/   Agent 实现层         │  ReAct / Reflection /
   │   只放范式的实现                │  Plan-and-Solve
   └──────────────┬───────────────┘
                  │ 依赖
                  ▼
   ┌──────────────────────────────┐
   │ core/     核心框架层           │  Agent 基类 / LLM 统一接口
   │   不含任何具体范式              │  消息系统 / 配置 / 异常
   └──────────────────────────────┘
                  ▲
                  │ 依赖
   ┌──────────────┴───────────────┐
   │ tools/    工具系统层           │  工具基类 / 注册机制 / 工具链管理
   │   只放工具的定义与调度           │  异步执行器 / 内置工具集
   └──────────────────────────────┘

「core 不认识 agents」是这套分层的关键约束：
core 里一旦出现具体范式相关的概念，层次就被压平了。
```
...
```
Message   对内丰富，对外兼容
   role 用 typing.Literal 锁死成四种取值（user / assistant / system / tool）
      └─ 非法角色在构造对象时就被挡住，而不是等发请求时才报错
   content / role 之外加了 timestamp 与 metadata
      └─ 为日志与后续扩展（多模态、工具调用元信息）预留位置
   to_dict() 是唯一出口，转成 OpenAI 兼容的字典
      └─ 内部想存多少存多少，发出去时只留对方认识的字段
   继承 pydantic.BaseModel，约束在赋值时就生效

Config    零配置可用，环境变量可覆盖
   每项都有合理默认值  ──▶  零配置也能跑起来
   from_env() 做覆盖   ──▶  换部署环境不用改代码
   max_history_length 是隐藏的关键项 —— 防止历史无界增长

Agent（抽象基类）  强制统一入口
   abc.ABC + @abstractmethod
      ├─ 不能直接实例化，必须实现 run()
      │     └─ 所有子类有统一执行入口：调用方不需要知道内部是 ReAct 还是 Reflection
      └─ 构造函数把依赖固定成四项：名称 / LLM 实例 / 系统提示词 / 配置
            └─ 依赖注入而非内部 new：换模型、换配置都不用碰子类
   三个历史方法在基类实现、子类复用
      └─ get_history() 返回 copy() 而非原列表：外部拿不到内部状态的引用
```
...
五种实现共用一套骨架，差异全在「循环体里做什么」。

```
                        Agent 基类
        （消息历史管理 / LLM 调用 / 配置读取 —— 公共部分全在这里）
                             │
                             ▼  统一入口 run()
   ┌──────────────────────────────────────────────────────────┐
   │ SimpleAgent         一次 LLM 调用，没有循环                 │
   │                                                            │
   │ ReActAgent          循环体：思考 → 选工具 → 执行 → 记录观察   │
   │                                                            │
   │ ReflectionAgent     循环体：执行 → 反思 → 优化               │
   │                     终止条件从「拿到答案」变成「反思无新问题」 │
   │                                                            │
   │ PlanAndSolveAgent   循环分两段：先一次调用生成计划，          │
   │                     再按计划逐步执行                        │
   │                                                            │
   │ FunctionCallAgent   循环体与 ReAct 同构，差别只在工具调用     │
   │                     由模型原生输出而非正则解析               │
   └──────────────────────────────────────────────────────────┘

差异部分的代码量远小于公共部分 —— 这就是抽象该起的作用：把变化的部分缩到最小。

反过来的判据：如果某个新范式无法用「循环体不同」来表达、而需要改基类的接口，
那说明抽象选错了层次 —— 这时该改的是基类，而不是打补丁。
```

...
**代价**：基于 LLM 的对话本质不确定，智能体可能偏离预期走向意外分支甚至循环；出问题时拿到的不是错误堆栈，而是一长串对话历史——「对话式调试」比读堆栈难得多。
...
`MsgHub` 是中枢，三个能力最值得记：**灵活路由**（点对点 / 广播 / 组播）、**消息持久化**（自动落 SQLite、MongoDB，长任务可恢复）、**原生分布式**（跨进程跨服务器靠 RPC 传递，对开发者透明）。

生命周期由统一基类 `AgentBase` 管理，开发者通常只需要实现 `reply(x: Msg) -> Msg`，可选实现 `observe`。**内部逻辑与外部通信被这个接口彻底分开了。**

一个值得注意的用法：把**流程建模成消息交互模式，而不是中心化状态机**。例如狼人杀里「狼人私聊」不是一次函数调用，而是用 `MsgHub` 动态创建一个只含狼人玩家的临时私密频道——**流程控制由通信拓扑表达，而不是由状态转移表表达**。

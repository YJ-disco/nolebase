...
智能体不是一次性完成任务，而是通过一个持续的循环与环境交互。这个机制叫 **Agent Loop（智能体循环）**。
...
**「幻觉工具调用」这一条值得强调**：模型的错误不是内容错，而是把「记不清的接口」当成「记得的接口」写出来（参见 [[07-模型幻觉]]）。护栏的关键是**返回结构化错误**而不是抛异常——错误信息进上下文，模型下一轮才有机会改对。

...
```
   ...
   ┌────────────────────────────────────────────────┐
   │ 抽象操作                                        │
   │   协议无关的能力：SendMessage、GetTask …          │
   └────────────────────────────────────────────────┘
   ┌────────────────────────────────────────────────┐
   │ 协议绑定（任选一种，在 Agent Card 里声明）         │
   │   ├─ JSON-RPC 2.0                              │
   │   ├─ gRPC                                      │
   │   └─ HTTP+JSON（REST）                          │
   └────────────────────────────────────────────────┘

第三层可选的原因很重要：A2A 不发明自己的线上格式。
   └─ 一个 A2A agent 因此能直接落进已有的 service mesh 或 API gateway，
      不需要特殊处理

底下是 JSON-RPC 2.0 加一层 SSE 做流式，外加可选的 webhook 做推送通知。
线上没有任何异乎寻常的东西 —— 任何一条报文都能格式化出来用眼睛读。
```
...
AgentCard 承担发现与自描述，可以发两张。

```
   公开 card（受限）
   /.well-known/agent-card.json      ← 标准路径，遵循 RFC 8615
   ├─ name / description / url / version
   ├─ skills                提供什么能力
   ├─ capabilities          streaming / pushNotifications / extendedAgentCard
   ├─ securitySchemes       接受哪些凭据（OpenAPI 规范的同一种形状）
...
注意旧路径 /.well-known/agent.json 已不是标准，很多材料还在用它。

两个刻意的设计选择
   ① card 是摘要级的，不枚举远端 agent 的 tools
      └─ 这正是 A2A 避开 MCP 那种上下文窗口问题的原因：
         若每个被发现的 agent 都把完整工具清单塞进来，
         光发现阶段就把模型的上下文撑爆了
   ② 可以发两张卡（见上）

客户端怎么找到这张卡
   直接取 well-known URI                    已知 agent 的地址
   查 curated registry                      需要发现未知 agent（按 skill 或 tag 索引）
   直接配置（config / 环境变量 / 私有 API）   固定关系
   └─ registry 对企业最关键 —— 它是最自然的那个
      「决定哪些 agent 允许被发现」的位置
```
...
Part 是原子内容容器，而 Message 与 Artifact 的分工容易看漏。
...
```
   ...
   │ Message            │         │ Artifact           │
   │ 临时的对话轮次       │         │ 留存下来的交付物      │
   │ role: user / agent │         │ artifactId         │
   │ messageId          │         │ name               │
   │ contextId          │         │ description        │
   │ referenceTaskIds   │         │ parts              │
   └────────────────────┘         └────────────────────┘

Artifact 的名字跨修订保持稳定
   一个 agent 在精修草稿时，返回的是同一个 artifact 的连续版本，
   而不是一堆互不相关的 blob。
   └─ 下游因此可以按名字跟踪「这份东西改到第几版了」

Task 是工作单元：id + contextId（把相关工作归到一组）+ status + history + artifacts
```

## Task 状态机：九个状态，两个是暂停不是失败
...
working ─────────────────────┬──────────────▶ completed       终态
   ...
   │                         ├──────────────▶ rejected        终态
   │                         │                agent 直接拒绝
   │                         ├──────────────▶ input-required  暂停，可恢复
   │                         │                需要客户端提供东西才能继续
   │                         ├──────────────▶ auth-required   暂停，可恢复
   │                         │                需要它没有的凭据
   │                         └──────────────▶ unknown         状态无法确定
   │
   └─ 中间两个不是失败：它们是刻意的暂停，把控制权交回客户端
      └─ 所以一个 A2A 任务是一次协商，而不是一次调用
         也让它天然适配 Human-in-the-loop：
         agent 不确定时不用猜，直接进 input-required 等回答
```
...
流式与推送的分岔，取决于任务时长和客户端的部署形态。

```
   任务时长
     几十秒 ────────────▶ 流式 message/stream
                           事件顺序：先发 Task 对象
                                    ──▶ status-update / artifact-update 事件
                                    ──▶ 到达终态时关闭流
                           前提：连接要一直挂着

     分钟到小时 ─────────▶ 推送 push notification
                           流式这时候不实用（连接挂那么久）
                           改用 webhook 回调替代轮询
                           注册：tasks/pushNotificationConfig/create

   客户端部署形态也参与判断
     客户端在浏览器里 + 任务几十秒 ──▶ 流式
     客户端是个服务   + 任务几小时 ──▶ webhook

规范里方法不少，真正干活的是五个
   message/send                       发消息，返回 Task 或 Message
   message/stream                     同上，响应是 SSE 事件流
   tasks/get                          按 id 取任务（可裁剪 history）
   tasks/cancel                       协作式地请求停止
   tasks/pushNotificationConfig/*     注册 webhook
   └─ 其余属于生活质量改进

报文里有个必须处理的分支：message/send 能返回两种东西之一
   短问答 ──▶ kind: "message"
   长任务 ──▶ kind: "task"
   判别字段就是 kind —— 客户端只按一种写，遇到另一种就会崩
```
...
```
   ...
   │ ANP    智能体 ↔ 大规模网络                         │
   │   线上：未稳定（概念框架，社区维护）                  │
   │   解决：怎么在大规模网络中发现和连接智能体             │
   └─────────────────────────────────────────────────┘
                        ▲
   ┌────────────────────┴────────────────────────────┐
   │ A2A    智能体 ↔ 智能体                            │
   │   线上：JSON-RPC 2.0 / gRPC / REST + SSE          │
   │   成熟度：有正式规范（v0.3.0）                      │
   │   解决：怎么和其他智能体对话                        │
   └────────────────────┬────────────────────────────┘
                        ▼
   ┌─────────────────────────────────────────────────┐
   │ MCP    智能体 ↔ 工具 / 资源                        │
   │   线上：JSON-RPC 2.0（stdio / Streamable HTTP）    │
   │   成熟度：生态相对成熟                             │
   │   解决：怎么访问工具                                │
   └─────────────────────────────────────────────────┘

真实系统里可以叠加：智能体内部用 MCP 接工具，对外用 A2A 与其他智能体协作；
网络规模大到需要动态发现时，再引入 ANP 那类机制。

与框架内编排的分界
   06 里那四种编排模型（AutoGen / AgentScope / CAMEL / LangGraph）解决的是
   同一个应用内部多个智能体怎么协作 —— 它们共享一个运行时，可以直接传对象。
   A2A 解决的是跨应用、跨平台 —— 双方没有共享运行时，
   所以必须发明一套自描述（AgentCard）、一套任务生命周期（九状态机）、
   一套异步协商机制（两个可恢复暂停状态）。这就是协议必要性的来源。
```

...
```
   ┌──────────────────────────────────────────────────────────┐
   │ MemoryTool（智能体记忆系统）                                │
   │   关注对话式记忆：短期工作记忆 / 情景记忆 / 语义记忆           │
   │   底层是向量库 + 图库 + 文档库，有嵌入与检索链路               │
   │   有 consolidate（短期转长期）这一步                         │
   └──────────────────────────────────────────────────────────┘
   ┌──────────────────────────────────────────────────────────┐
   │ NoteTool                                                  │
   │   服务项目式任务：长期追踪、结构化管理                        │
   │   更轻、更人类友好                                          │
   │   Markdown + YAML：机器可解析，人也易读易改                   │
   │   纯文本天然进 Git；不走数据库，适合轻量状态追踪                │
   │   用 type + tags 两个维度组织，支持多维检索                    │
   └──────────────────────────────────────────────────────────┘

   一句话：MemoryTool 管「我经历过什么」，
   NoteTool 管「这个项目走到哪一步了、下一步做什么」。
```
...
```
   ...
   blocker      遇到的问题和阻塞点
        └─ 需要被反复拉回上下文，直到解决为止   ← 与 action 的关键区别

   action       下一步的行动计划
        └─ 完成一次就可以丢弃

   └─ blocker 单独成一类是有道理的：它的生命周期与 action 相反 ——
      阻塞项要一直占着上下文，而行动项做完就出局。
      把两者混成一类，会导致要么阻塞项被冲掉、要么已完成项一直占位。

同一套机制也适用于非编码场景
   研究任务里记每篇文献的核心观点（conclusion）、
   待调研主题（action）、重要参考文献（reference）
```
...
每个笔记是一个独立 `.md` 文件，索引用 JSON 集中存元数据。

```
   notes/note_20250119_153000_0.md
   ┌─────────────────────────────────────┐
   │ ---                                 │  ← YAML 元数据（机器可解析）
   │ id: note_20250119_153000_0           │
   │ title / type / tags                  │
   ...
   └─────────────────────────────────────┘
                  ...
                  ▼
   notes_index.json
   { "note_…": { id, title, type, tags, created_at, updated_at, file_path } }

   三个设计点
     文件名即 ID    ──▶ 不需要额外维护一张 id → 文件的映射表
     正文存文件、索引只放元数据 + file_path
     id 形如 note_<YYYYMMDD>_<HHMMSS>_<序号>
       └─ 序号那一位是给「同一秒内创建多条笔记」用的 —— 时间戳精度不够时的兜底

   索引的三个作用与一个代价
     快速检索      不用打开每个文件，直接从索引里按 type / tags 过滤
     元数据管理    集中管理所有笔记的元数据
     完整性校验    索引里有但文件不在，就是不一致
     └─ 代价：索引与文件是两份数据，必须同步维护 ——
        create / update / delete 三个操作都要同时改两处，
        漏一处就出现「索引说有、文件没有」的漂移
```
...
```
   create  ┐
   read    │
   update  ├─ 对应记忆五阶段里的：编码 / 存储 / 检索 / 遗忘
   search  │
   list    │   create / update ──▶ 编码 + 存储
   summary │   search          ──▶ 检索
   delete  ┘   delete          ──▶ 遗忘
        │
        └─ 没有「整合（Consolidation）」这一步
           └─ NoteTool 是纯结构化的外部存储，不做「短期转长期」
              那一步是 MemoryTool 的 consolidate 负责的

接进 ContextBuilder 时有一个数字值得注意
   笔记包的 relevance_score 被赋成 0.7，而不是让它按默认的 0.5 走
   └─ 这是在告诉 Select 阶段「主动检索命中的笔记比一般候选更值得占预算」——
      它已经过了一轮检索筛选，命中本身就携带信息
```
...
允许智能体执行命令能力很强，安全机制做成四层。

```
   ① 命令白名单
        只放行安全的只读命令，完全禁止任何可能修改系统的操作
        判据是「只读」：列表 / 查看 / 搜索 / 文本处理全部只读；
        写入、删除、权限变更、网络请求一律不在表里
        拒绝时要报出允许列表 —— 这是给模型看的，
        它下一轮可以据此换一个合规命令，而不是继续试
        │
        ▼
   ② 工作目录限制（沙箱）
        只能访问指定工作目录及其子目录
        ...
        ├─ 不允许：cat /etc/passwd
        └─ 不允许：cd ../../../etc     ← 这条是防线的关键
           只挡绝对路径的沙箱等于没挡，.. 一路退出去照样能读系统文件。
           沙箱必须在路径解析之后做校验，而不是在字符串层面匹配前缀
        │
        ▼
   ③ 超时控制      每个命令有执行时间上限，防无限循环与资源耗尽
        │
        ▼
   ④ 输出大小限制   限制命令输出体积，防内存溢出

   ③ 与 ④ 对应 Agent Loop 里那三道硬上限中的两道：
   超时抓「卡住的工具调用」，输出上限抓「单次调用吃光内存」。
   区别在于这里把它们下沉到了工具层 ——
   不依赖上层循环自觉，工具自己就拒绝执行。

   四层合起来的设计意图：即使智能体的行为出现异常，
   也无法影响系统其他部分。
```

...
```
   ┌─────────────────────────────────────────────────────────┐
   │ 用户 harness（你建的）                                     │
   │   AGENTS.md · skills · hooks · 自定义 linter ·             │
   │   review agent · CI sensors                              │
   │                                                          │
   │   ┌─────────────────────────────────────────────────┐    │
   ...
   Model          是内核
   内层 harness   厂商围绕它建（system prompt、工具集、代码检索机制）
   外层 harness   你围绕内层建，为自己的代码库定制

Harness Engineering 讲的基本都是最外那一圈。建得好的外层 harness 做两件事：
   ① 提高 agent 第一次就做对的概率
   ② 提供自我修正的反馈回路，让很多问题在到达人之前就被处理掉
      └─ 减少审查负担、提高质量、少烧 token
```
...
harness 像一个调速器，用两类控制把代码库调向期望状态。

```
   Guides（前馈控制）              Sensors（反馈控制）
   时机：agent 行动之前             时机：agent 行动之后
   作用：预测它的行为并引导它，       作用：观测结果并帮它自我修正
        提高一次做对的概率
   例子：编码约定文档 / AGENTS.md /  例子：类型检查 / linter / 测试 /
        skills / 参考文档 /         静态分析 / AI review agent
        how-to 指南 / codemod

        └────────── 两者缺一不可 ──────────┘
   只有 guides 没有 sensors
      └─ harness「编码了规则，却永远不知道规则是否奏效」
   只有 sensors 没有 guides
      └─ agent「一直重复同样的错误再修正」

每种控制又有两种执行形态，按需选而不是越强越好
   Computational（确定性）
     快、便宜、可预测、CPU 跑；成本在毫秒到秒级
     └─ linter、结构测试、格式检查、类型检查
   Inferential（推理性）
     语义理解强、较慢、非确定性；成本高
     └─ LLM-as-a-judge、风格审查、需求符合度检查
   └─ 确定性检查放在最前面（快且可靠），
      推理性检查放在后面（贵，但能做 linter 做不到的判断）

一个精妙的设计：让 sensor 说模型能听懂的话
   一条自定义 linter 消息不只说「error」，而是包含怎么修的指令。
   Böckeler 称这个叫「一种正向的 prompt injection」——
   sensor 用 agent 能行动的语言对它说话。
   └─ 同一个原理在别处也出现过：ReAct 把解析失败的错误信息塞回历史、
      Function Calling 校验失败返回描述性错误而不是抛异常。
      错误信息是给模型的输入，所以要按输入的规格来写。
```
...
```
   ┌──────────────────────────────────────────────────────────┐
   │ 可维护性 harness    管内部代码质量                          │
   │   成熟度：最容易 —— 工具成熟                               │
   │   （linter、复杂度分析、测试覆盖）                          │
   └──────────────────────────────────────────────────────────┘
   ┌──────────────────────────────────────────────────────────┐
   │ 架构适配 harness    管性能、可观测性等架构特性                │
   │   （Fowler 的 fitness functions）                          │
   │   成熟度：中                                               │
   └──────────────────────────────────────────────────────────┘
   ┌──────────────────────────────────────────────────────────┐
   │ 行为 harness        管应用是否真的做了它该做的事              │
   │   成熟度：最难 ——「房间里的大象」，基本未解                   │
   └──────────────────────────────────────────────────────────┘

   第三类为什么难
     AI 生成的测试套件目前还不够可靠
     （对应 AI Coding 工作流那篇里「覆盖率 90% 但只测了 happy path」）
     └─ 用模型验证模型，会撞上同源盲区 ——
        这是 LLM Evaluation 与反馈闭环要单独处理的问题
```
...
```
   ┌──────────────────────────────────────────────────────────┐
   │ 工具与执行层                                               │
   │   读写文件、调用 Bash、访问 API、操作浏览器 —— 工具定义与授权  │
   │   └─ 工具表面越清晰，Agent 行为越可预期                      │
   └──────────────────────────────────────────────────────────┘
   ┌──────────────────────────────────────────────────────────┐
   │ 控制与验证层（Harness 区别于 Prompt 工程的核心）              │
   │   对照：让 Agent「请遵守代码规范」          → 概率性合规       │
   │         接一个违反规范就阻断 PR 的 Linter   → 确定性约束       │
   │   测试套件 / 类型检查 / 权限边界 / 审批门控接进来之后，
   │   这类错误从「概率上的减少」变成「结构上的关闭」——
   │   它不再有机会进入主干                                      │
   └──────────────────────────────────────────────────────────┘
   ┌──────────────────────────────────────────────────────────┐
   │ 持久化与状态层                                             │
   │   长任务的瓶颈是上下文窗口一满、之前的工作就丢了               │
   │   └─ 用文件系统和 Git 把状态落盘，Agent 才可能从中断处续跑     │
   └──────────────────────────────────────────────────────────┘

另一套流传较广的划分是三个支柱：上下文工程 / 架构约束 / 熵管理。
   └─ 这套说法来自中文技术自媒体，与 Böckeler 的 guides / sensors
      并非同一套划分，未见官方对应，标为待验证
```

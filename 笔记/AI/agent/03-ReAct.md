...
ReAct 的结构核心是把语言空间并进动作空间。

```
决策被形式化为在 A ∪ L 中选择
   A  环境动作集合（Search、Calculator、API 调用……）
   L  语言空间
   └─ 于是「产生一条 Thought」本身就是一次 action

时间步 t 的策略与工具
   (th_t, a_t) = π( q, (a_1, o_1), …, (a_{t-1}, o_{t-1}) )     ← 模型决定思考与行动
   o_t = T(a_t)                                                 ← 工具执行并返回观察

   新的 (a_t, o_t) 追加进历史，循环直到 th_t 里判断任务完成

为什么这个扩充是必需的：thought 与 action 走的是同一条 token 流、
同一种「输出」。模型不需要在两种模式之间切换接口，才可能在一次生成里
交替输出两者 —— 这是「思考与行动相辅相成」能成立的结构前提。

两个方向的提法
   reason to act   思考指导行动：诱导、追踪、调整计划
   act to reason   行动校正思考：带回外部事实，压住参数化记忆里的幻觉
```
...
模型输出（纯文本）
   │
   ▼
第一级 _parse_output：把两个字段切开
   Thought: 匹配到 Action: 或文本末尾
   Action : 匹配到文本末尾
   │
   ▼
   ┌──────────────────────┬────────────────────────────┐
   │ thought 字符串        │ action 字符串               │
   └──────────────────────┴─────────────┬──────────────┘
                                        │
                                        ▼ 第二级 _parse_action
                        用 (\w+)\[(.*)\] 拆成工具名与输入
                ...
                ┌───────────────────────┴───────────────────────┐
                ▼                                               ▼
        action 以 Finish 开头                           其余情况
                │                                               │
        直接提取最终答案并 return                     从工具字典按名取函数执行
                │                                               │
        （判定必须在查表之前 ——                        返回的原始数据由感知系统
          不能等查表时才发现                             封装成简洁的自然语言 Observation
          没有名为 Finish 的工具）                      作为下一轮的主要输入
```

这套「prompt 约束格式 → 正则解析 → 字典分发」就是 LangChain、LlamaIndex 这类框架早期的设计内核。更可靠的做法是原生 Function Calling，不再依赖正则去猜。
...
单独看任何一个数字都会得出错结论，三块要一起看。

```
HotpotQA（EM，6-shot）—— 差距小，甚至略输
   CoT               29.4
   ReAct             27.4   ← 单独用 ReAct 略输 CoT
   ReAct + CoT       35.1   ← 结合最好
   有监督 SoTA       67.5（用约 140k 训练样本）

FEVER（acc，3-shot）—— 反超
   CoT               56.3
   ReAct             60.9
   ReAct + CoT       64.6
   有监督 SoTA       89.5（用约 90k 训练样本）

ALFWorld（成功率）—— 差距最大
   Act-only          45
   ReAct             71     ← 只用了 2 个示例
   模仿学习基线       37     ← 用了约 100k 样本
                     └─ 绝对提升 34 个百分点
...
结论的形状：prompting 加几个示例，补上了大部分需要 GPU-months 训练才能拉开的差距，
           在动作空间大、需要探索的交互式任务上直接反超。
           ReAct 不宣称打败全监督基线 —— 在 HotpotQA / FEVER 上差得远。
```
...
幻觉           ██████████████████ 56%   幻觉       ██ 6%
推理错误       █████ 13%                 推理错误   ████ 13%
（无外部交互，错了也发现不了）           Search error ███████ 23%
                                         Label 歧义  █ 5%
                                         Other      █ 3%
                    └─ ReAct 把幻觉从 56% 压到 6%，几乎清零

但它没有消灭失败，只是把瓶颈换了位置：
   23% 的失败来自外部工具本身的质量（搜索没找到正确页面）
   13% 是页面找对了但 Thought 里的归因 / 计数 / 时序推理错了

这条结论直接推出了后续一批工作：与其优化推理，不如优化检索器。
```

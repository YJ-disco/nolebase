...
client-host-server 架构：一个 host 跑多个 client 实例，每个 client 与一个 server 1:1 隔离。

```
   ┌───────────────────────────── Host ──────────────────────────────┐
   │ 创建并管理多个 client 实例 / 控制连接权限与生命周期                 │
   │ 执行安全策略与同意要求 / 处理用户授权                              │
   │ 协调 LLM 集成与采样 / 聚合跨 client 的上下文                       │
   │                                                                 │
...
第 3 条设计原则是安全模型的基石：**隔离发生在协议层，不是靠约定** ——
server 读不到整段对话，也看不进其他 server。
```
...
② 服务器 → result
   protocolVersion + capabilities + serverInfo + instructions（可选）

③ 客户端 → notifications/initialized
   无 id、不期待响应 —— 它标志握手完成

   两条硬约束把「谁先动」卡死了
     客户端：服务器响应 initialize 之前，除 ping 外不发任何请求
     服务器：收到 initialized 之前，除 ping 与 logging 外不发任何请求
     └─ 避免双方在能力尚未确定时就开始调用

版本协商三条规则，顺序不能错
   ① 客户端必须在 initialize 里带上自己支持的最新协议版本
   ② 服务器支持它 ──▶ 必须回同一个版本
      不支持      ──▶ 必须回一个自己支持的版本
   ③ 客户端收到后若不支持那个版本 ──▶ 应该断开
   └─ HTTP 传输下后续请求必须带 MCP-Protocol-Version 头；
      服务器收不到且无从推断时应该假定 2025-03-26

关闭阶梯（协议没有专门的关闭消息，靠底层传输表达）
   stdio：客户端先关闭子进程的输入流 ──▶ 等服务器退出
        ──▶ 不退则 SIGTERM ──▶ 再退不出则 SIGKILL
        （服务器也可以关掉自己的输出流并退出）
   HTTP ：关闭对应的 HTTP 连接

超时：所有请求都应该有超时，超时后应该发取消通知并停止等待。
      收到进度通知时可以重置时钟（说明工作确实在推进），
      但应该始终存在一个最大超时。
```
...
三个原语按「谁控制」区分，这是理解它们的钥匙。

```
   ┌─────────────────────────────────────────────────────┐
   │ Tools        model-controlled                       │
   │   模型自主发现并调用                                  │
   │   列表：tools/list（支持分页，带 nextCursor）          │
   │   调用：tools/call {name, arguments}                 │
   │   结果：result.content 数组 + isError                │
   └─────────────────────────────────────────────────────┘
   ┌─────────────────────────────────────────────────────┐
   ...
   │   用户显式选择，典型形态是斜杠命令                      │
   │   prompts/get 返回的是一条可直接喂给模型的 messages    │
   │   数组，不只是模板字符串                               │
   └─────────────────────────────────────────────────────┘

三者的子能力都有 listChanged（列表变化时发通知），subscribe 只有 resources 有。
两者都是可选的：可以都不支持、只支持其一、或都支持。

注意方向：sampling 与 elicitation 是服务器向客户端发请求 ——
MCP 是双向的，不是单纯的 client → server 调用。
```
...
工具错误不走 JSON-RPC 错误通道，两条路必须分开。

```
调用本身不成立（协议错误）              调用成立、业务失败（工具执行错误）
   {                                     {
     "jsonrpc": "2.0", "id": 3,             "jsonrpc": "2.0", "id": 4,
     "error": {                             "result": {
       "code": -32602,                        "content": [{ "type": "text",
       "message": "Unknown tool:                 "text": "API rate limit" }],
               invalid_tool_name"             "isError": true
     }                                      }
   }                                      }
   └─ 走顶层 error                         └─ 走 result.isError
   └─ 客户端该修的 bug                      └─ 要交给模型理解并重试

区别的意义：后者必须以正常结果的形式回到对话里，才能成为模型的输入。
若走 error 通道，模型根本看不到它 —— 也就失去了自我纠正的机会。

完整错误码
   -32602   参数无效（含未知工具名、提示名无效、缺必需参数）   顶层 error
   -32002   资源不存在                                       顶层 error
   -32603   内部错误                                         顶层 error
   -1       用户拒绝采样（实现自定义）                          顶层 error
   ——       工具执行失败                                      result.isError
```
...
客户端                          服务器（作为子进程启动）
  ...
  │                               │ 从 stdin 读报文
  │  ◀── stdout ──                │ 往 stdout 写报文
  │  ◀── stderr ──                │ 可往 stderr 写 UTF-8 日志
  │                               │ （客户端可捕获、转发或忽略）

四条硬约束
   报文之间用换行分隔，消息内部不能含内嵌换行
   Server 不得向 stdout 写任何非 MCP 报文
   客户端不得向 Server 的 stdin 写任何非 MCP 报文
   日志一律走 stderr

   最后一条是最常见的实现事故来源：Server 代码里留一个 print()，
   或某个依赖往 stdout 打日志，报文流就被污染，客户端解析直接失败。

客户端应该尽可能支持 stdio —— 它是本地集成成本最低的一种。
```
...
Streamable HTTP：单一 endpoint 同时支持 POST 与 GET。
...
```
              客户端                          服务器（独立进程，可服务多连接）
                ...
                ├───────────────────────────────▶│
                │                                │
   Body 是 request ──▶ 服务器必须二选一返回
                │◀── Content-Type: text/event-stream（开 SSE 流）
                │    或 Content-Type: application/json（单个 JSON 对象）
                │    └─ 客户端必须两种都支持
                │
   Body 是 response 或 notification
                │◀── 202 Accepted 且无 body（接受）
                │    或 HTTP 错误码（如 400，body 里放无 id 的 JSON-RPC 错误）
                │
                │  GET /mcp                      │
                ...
                │    └─ GET 开的流上不得发 JSON-RPC 响应（续传除外）

几条容易踩的规则
   断连不等于取消 —— 要取消得显式发 CancelledNotification
   多连接：客户端可同时连多个 SSE 流；
          服务器必须只把每条消息发在其中一条流上，不得广播
   续传：事件 ID 是每条流各自的游标 —— 带 Last-Event-ID 重连时，
        不得重放属于其他流的消息

安全要求三条不能省
   必须校验所有入站连接的 Origin 头（防 DNS rebinding）
   本地运行时应该只绑 127.0.0.1
   应该为所有连接实现认证
```
...
没有网关：M 个客户端 × N 个服务器
   ...
   每台开发者机器上放着各自的凭据、没有统一审计、
   也没法按人控制能用哪些 server

有网关：认证、策略、可观测性的唯一执行点
...
关键区分：MCP 网关 ≠ LLM 网关
   LLM 网关代理模型供应商，管模型调用的路由与计量
   MCP 网关代理工具（MCP Server），管从 agent 到工具调用的治理

...
零拷贝（Zero-Copy）技术是一种减少数据拷贝次数的 I/O 操作方式，它通过减少数据在计算机系统中的拷贝次数，从而提高数据传输的效率并降低 CPU 的负载。在传统的 I/O 操作中，数据通常需要在用户态和内核态之间多次拷贝，而零拷贝技术通过优化这些操作，减少了不必要的数据拷贝，从而显著提升了性能。

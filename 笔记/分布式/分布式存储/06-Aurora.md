...
论文对环境的判断是这个设计的起点：现代云服务通过**把计算与存储解耦**来获得韧性与可扩展性。一旦解耦，**传统数据库的 I/O 瓶颈就换了位置**：
...
论文还点出同步操作的代价：**一次缓冲池未命中导致的磁盘读就是同步的** —— 读取线程必须等它完成，甚至可能还要为腾出页面而驱逐并刷写一个脏页。后台处理（如 checkpoint）能减少这种情况，但**后台处理本身也会造成 stall**。
...
常见做法是 $V = 3$、$V_w = V_r = 2$。**论文明确说 2/3 法定人数不充分。**
...
在 2/3 模型下，此时**已经丢了两份**，**无法判断第三份是否最新**。论文把本质点出来了：
...
论文还补了一句链条：**保证读法定人数，使我们可以通过增加额外副本把写法定人数重建起来。**
...
论文用**跨 AZ 同步镜像的主备 MySQL** 举例，列出引擎实际要写的**五类数据**：
...
- **从"怎么写"看**：步骤 1、3、5 是**顺序且同步**的，所以**延迟是叠加的**；而且**即使异步写也必须等最慢的那个操作**，于是**抖动被放大，系统受离群点摆布**。论文给了个很准的定性：**这个模型相当于 4/4 的写法定人数**（四份都要写成功），对故障与离群性能都脆弱。
...
论文用 SysBench 的**只写负载 + 100 GB 数据集 + 30 分钟 + r3.8xlarge** 对比了两种配置：
...
论文还给了一个"用 CPU 换磁盘"的具体例子：**存储节点忙于处理前台写请求时，不必对旧页面版本做垃圾回收 —— 除非磁盘快满了。**
...
第 4 节回答"日志怎么从引擎生成，使持久状态、运行时状态、副本状态始终一致" —— **关键是不用昂贵的 2PC**。论文对 2PC 的评价是**"话多（chatty）且不容忍故障"**。
...
| 为什么不用 2PC | 论文评价它**"话多且不容忍故障"**；改用**单调递增 LSN + 点推进 + gossip 补缺** |

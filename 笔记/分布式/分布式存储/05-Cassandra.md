...
论文对需求的描述很具体：Inbox Search 要**每天处理数十亿次写**，还要随用户数扩展；用户从**地理分布的数据中心**被服务，所以**跨数据中心复制**是压低搜索延迟的关键。系统 **2008 年 6 月上线时约 1 亿用户，论文写作时超过 2.5 亿**，并已成为 Facebook 内多个服务的后端。

设计目标一句话：**跑在廉价商品硬件上，处理高写吞吐而不牺牲读效率。**

## 数据模型：Bigtable 那套，加一层 super column

论文自己说这个数据模型**与 Bigtable 非常相似**。两处不同：
...
论文还顺带交代了一条部署现实：**系统支持多表的概念，但所有部署的 schema 里只有一张表**。
...
**但 Cassandra 选的解法与 Dynamo 不同。** 论文明确列出两种方式：
...
**为什么需要它**：**节点负责范围的元数据缓存在本地，同时以容错方式存在 ZooKeeper 里** —— 这样**崩溃后重启的节点知道它原先负责哪些范围**。论文用了 Dynamo 的说法，把一个范围对应的节点集合称为该范围的 **preference list**。
...
**这里有一个实现层面的发现**：原论文建议用**高斯分布**近似这个间隔分布，**但 Cassandra 发现指数分布是更好的近似** —— 理由是 **gossip 通道的特性及其对延迟的影响**。论文还声明：**在他们所知范围内，这是第一个在 Gossip 场景下实现 Accrual Failure Detection 的工作**。

论文对它的评价是**准确性与速度都很好，且能很好地适应网络条件与服务器负载条件**。
...
**扩容的实测数字**：新节点被分配一个 token 以分担某个重载节点，于是**切分**了原本由另一节点负责的范围；**让出数据的节点用内核到内核拷贝技术把数据流给新节点，单节点速率可达 40 MB/sec**。论文说正在改进的做法是**让多个副本参与 bootstrap 传输以并行化，类似 Bittorrent**。
...
- 盘上文件多了之后，**一个后台合并进程把它们收拢成一个** —— 论文自己点明**这与 Bigtable 的 compaction 过程非常相似**；
...
| 分布用什么近似 | **指数分布**（比原论文建议的高斯分布更贴合 gossip 通道的延迟特性） |
...
- [[04-Zab 与 ZooKeeper|Zab 与 ZooKeeper]] —— 论文里的 ZooKeeper 用它做什么（存范围元数据、选 leader 维持不变式）
- [[03-一致性模型]] —— 「按客户端要求的一致性保证」调整读路径，是最终一致谱系里的典型做法

## 参考

- Avinash Lakshman, Prashant Malik. *Cassandra: A Decentralized Structured Storage System*. 2009（LADIS 2009）.（Inbox Search 的需求、Super column family 数据模型与三个 API 方法、保序哈希、三档复制策略、ZooKeeper 的角色、Accrual Failure Detector 与 $\phi$ 的刻度表、bootstrap 的显式触发与 40 MB/sec、本地持久化的全套参数）

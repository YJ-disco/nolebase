...
论文开头给的规模锚点：**650 亿张照片**（论文时），每张照片生成 **4 种尺寸**，合计 **2600 亿张图、超过 20 PB**；**每周新增 10 亿张（约 60 TB）**；**峰值每秒服务超过 100 万张图**（这些数字和论文里的日流量表口径不同，前者是累计存量与峰值，后者是单日）：
...
被浏览的尺寸分布也很说明问题：缩略图 **10.2%**、小图 **84.4%**、中图 **0.2%**、大图 **5.2%**。**八成以上是小图**，而小图恰好是 News Feed 里用的、对延迟最敏感的那类 —— 论文据此强调元数据开销必须压小，因为**单张图的元数据成本会被这个量级的请求数放大**。

## 要解决的问题：一次读取要花几次磁盘 I/O

上一版系统是 **NFS 挂载的商用 NAS**，每张照片一个文件。论文把问题链一步步量了出来：

1. **每目录放几千个文件** → NAS 设备的**目录 blockmap 大到无法被缓存** → **单张图常见超过 10 次磁盘操作**；
2. 把每目录降到**几百个文件** → 仍然稳定要 **3 次**：① 读目录元数据进内存；② 读 inode 进内存；③ 读文件内容本身；
3. 让 Photo Store 服务器**用 memcache 缓存 NAS 返回的 file handle**，并为此给内核加了 `open by filehandle` 系统调用 → **改善很小** —— 因为**冷门照片本来就不会被缓存**。论文顺势点掉了"把所有 file handle 都塞进 memcache"这条退路：它只解决了一半问题，**仍然依赖 NAS 设备把全部 inode 驻留内存**。

> [!warning] CDN 不能替代后端存储
> 论文把这条写成了"主要教训"：CDN 擅长服务**最热的内容**（头像、刚上传的照片），但社交网站还会产生大量**冷门（常常是旧的）内容请求**，即**长尾**。长尾请求**几乎全部落在后端存储上**（它们通常在 CDN miss），而把长尾全部缓存起来**不划算**（缓存容量要求太大）。

四个设计目标也就此确定：**高吞吐低延迟**（每次读取最多一次磁盘操作）、**容错**、**低成本**、**简单**（论文特意强调简单性的分量：正因如此，**几个月而不是几年**就建成并上线）。
...
三层容量单位的关系是：**物理卷**（physical volume，**100 GB** 一个）→ 同一台机器上的多个物理卷构成一块容量（论文举例：**10 TB 的服务器切成 100 个物理卷**）→ **跨机器的物理卷再组成逻辑卷**（logical volume），**照片写进一个逻辑卷时会写到它对应的所有物理卷**。冗余就发生在这里。
...
3. **决定一个请求该走 CDN 还是走 Cache** —— 论文说明这个功能让人**能主动调节对 CDN 的依赖程度**；
...
论文顺带提到一个**计划中的优化**：既然刚上传的照片马上会被读很多次，不如**主动把新上传的照片推入 Cache**。
...
**那一对 (key, alternate key) 的用法值得说明**：上传时 web 服务器把同一张照片**缩成 4 种尺寸**，写成 **4 个 needle，key 相同**；**alternate key 用来区分尺寸** —— 论文只给出取值范围：**按递减顺序可为 `n`、`a`、`s`、`t`**。
...
于是 **Haystack 在读照片时可以把文件系统元数据那几次磁盘操作完全消掉**。论文同时给了一个诚实的边界：**这不保证每次读恰好一次磁盘操作** —— **照片数据跨越 extent 或 RAID 边界时，文件系统仍可能要多花一次**。所以 **Haystack 预分配 1 GB 的 extent、并把 RAID stripe 设成 256 KB**，使这些情况在实践中很少遇到。
...
论文把账算到底：每张上传的图缩成 **4 张**，**key 64 位 + alternate key 32 位 + data size 16 位，共 32 字节**，再加**哈希表开销约 2 字节** → **同一张图的 4 个尺寸合计 40 字节**。
...
- **bulk sync**：用**副本提供的卷文件**重置一台 Store 机器的数据。**每月只发生几次**，简单但慢 —— **瓶颈是要同步的数据量常常比该机器网卡速率大好几个数量级，恢复时间以小时计**。论文说明正在探索改进办法。

## 评测

**Cache 命中率约 80%** —— 因为 Cache 只存**可写机器上的照片**，而那些照片是较新的，正好落在访问密集的区间。论文强调这使 Cache**显著降低了那批本会受影响最大的机器的读请求率**。
...
负载 A 的解读：**Haystack 达到裸设备吞吐的 85%，延迟只高 17%**。开销来自四处，论文逐条列出：① 跑在文件系统之上而非直接访问磁盘；② 磁盘读的**比 64 KB 大**，因为要整块读 needle；③ 存的图**可能没对齐 RAID-6 stripe**，一小部分图要从多块盘读；④ Haystack 服务端的 **CPU 开销**（索引访问、校验和计算等）。
...
论文用两个口径量化收益：**每可用 TB 的成本**与**每可用 TB 的读速率**。结论是**每可用 TB 便宜 28%，且每秒处理的读次数是 NAS 方案的约 4 倍**。
...
- **Ceph**（本专栏后续会收）—— 论文的 related work 里点名了 Ceph 的做法：**用生成函数代替显式映射**（客户端直接算元数据而不是查表），并把"在 Haystack 里实现该技术"列为 future work

## 参考

- Doug Beaver, Sanjeev Kumar, Harry C. Li, Jason Sobel, Peter Vajgel. *Finding a Needle in Haystack: Facebook's Photo Storage*. OSDI 2010.（规模锚点与日流量、NFS/NAS 方案的 I/O 问题链与 CDN 教训、Store/Directory/Cache 三组件与 URL 寻址、needle 字段与 8 字节对齐、内存映射与 cookie、追加写的版本语义、index file 的 orphan 与陈旧 record、XFS 与 1 GB extent / 256 KB stripe、10 字节内存账本与 536 字节 inode 对照、compaction 与 25% 删除率、pitchfork 与 bulk sync、Cache 的两条缓存条件与 80% 命中率、合成与生产评测、28% 成本与 4 倍读速率）

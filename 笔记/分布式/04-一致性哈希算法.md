...
我们知道，传统的按服务器节点数量取模在集群扩容和收缩时存在一定的局限性。而一致性哈希算法正好解决了简单哈希算法在分布式集群中存在的动态伸缩的问题。降低节点上下线的过程中带来的数据迁移成本，同时节点数量的变化与分片原则对于应用系统来说是无感的，使上层应用更专注于领域内逻辑的编写，使得整个系统架构能够动态伸缩。

### 一致性哈希的使用场景

一致性哈希算法是分布式系统中的重要算法，使用场景也非常广泛。主要是是负载均衡、缓存数据分区等场景。

一致性哈希应该是实现负载均衡的首选算法，它既可以在客户端实现，也可以在中间件上实现，比如日常使用较多的缓存中间件memcached 使用的路由算法用的就是一致性哈希算法。
...
- **`Math.abs` 对 `Integer.MIN_VALUE` 无效**：补码下该值没有对应的正数，`Math.abs(Integer.MIN_VALUE)` 返回的仍是负数，`if (hash < 0)` 这一层的意图在这个取值上落空。
- **`(int) 2166136261L` 的截断是有意的**：FNV 的 offset basis 在 32 位补码下等于 `0x811C9DC5`，作为有符号 int 是负数——环上的位置按这套有符号语义生成，前后自洽；把它当无符号下标解读就会错位。

环的语义是无符号的 $[0, 2^{32})$，实现的键是有符号 `int`。生成、比较、查找三处保持同一语义，映射关系才稳定。
...
前面讲的原理与实现都不是凭空来的。一致性哈希出自 Karger 等人在 STOC 1997 上的工作（原题见「参考」）。

它最初要解决的是 **Web 代理缓存的热点**，而非「缓存怎么分片」这类通用问题：某些页面会在极短时间内被大量请求 —— 1994 年 Shoemaker-Levy 9 彗星撞击后的 JPL 站点、1997 年 IBM 的深蓝比赛页面，都属于把单台服务器或共享代理缓存压垮的情形。而当时通行的「对缓存机器数 $n$ 取模」在机器增减时会重排几乎所有 key，等于把请求全打回源站，与缓解热点的目的正好相反。

### 四个评判指标

这里先定义**一个方案该满足什么**，再给算法。这四个指标可以直接用来判断任何一种分片方案：

| 指标 | 定义 | 违反它的后果 |
| --- | --- | --- |
| **平衡性（Balance）** | 不同 key 的哈希结果尽可能均匀落到各节点上 | 全部 key 挤在一台 |
| **单调性（Monotonicity）** | 可用节点集合变化时，key **要么留在原节点、要么落到新加入的节点**；不会从一个老节点挪到另一个老节点 | 加一台机器导致两台老机器之间的数据对调 |
| **分散性（Spread）** | 同一个 key 在不同客户端视角下被分配到的节点集合尽量小 | 客户端看不到全部节点 → 同一 key 落在不同节点，同一份数据被缓存多份，存储效率下降 |
| **负载（Load）** | 从服务器视角看，单台节点承担的 key 数量尽量少 | 一台节点要存远超均值的 key |

**单调性是这里最关键的一条**，它把「一致性」这个词说清楚了：扩容缩容时受影响的 key 应当局限在**换手的那一段**，而不是全局重排。这也解释了为什么对 $n$ 取模的方案不合格 —— 它连平衡性都满足不了（$n$ 一变，几乎每个 key 的落点都变）。

### 重映射界

传统哈希在槽数变化时**几乎所有 key 都要重映射**。一致性哈希给出的界是

$$\text{平均重映射的 key 数} = \frac{K}{n}$$

$K$ 是 key 总数，$n$ 是节点数 —— 即**新增一个节点平均只搬走它应得的那一份**。

### 虚拟节点的最初形态

机制是：**每个节点（桶）映射到的是一系列伪随机的点，而不是环上的单个点**。这个设计承担两个作用：

1. **削弱数据倾斜**：一个节点的多个点把它的「份额」分散到环上各处，各节点分到的区间之和因此更接近均值 —— 这正是前面「数据倾斜与虚拟节点」一节在数学上的依据；
2. **可调节比例**：**改变某个节点映射的点数，就能改变它承担的 key 比例**。这让异构集群（机器容量不同）可以按容量分配负载，而不必假设每台机器一样大。

所以「虚拟节点」不是后来的工程补丁，它在原始方案里就有 —— 本笔记「一致性哈希算法实现」一节正是这个机制。

> **来源核对**：以上四个指标、「每个桶映射到多个点」与重映射界三点，已对照 STOC 1997 的正文逐条核实；精确表述、定理编号与评估口径见下。

### 精确表述与定理

**模型**：哈希函数带一个 view 参数。view 是客户端可见的桶集合，各客户端的 view 可以不一致。STOC 1997 把一致性哈希形式化为 **ranged hash function** $f: 2^B \times I \to B$——给定 view $V$ 与条目 $i$，输出 $f_V(i) \in V$。四个指标都是对这个函数族的要求：

| 指标 | 精确表述 |
| --- | --- |
| 平衡性 | 对固定 view $V$，$\Pr[f_V(i) = b] = O(1)/|V|$——每个桶分到的条目比例以高概率是 $O(1/|V|)$ |
| 单调性 | $V_1 \subseteq V_2$ 且 $f_{V_2}(i) \in V_1 \Rightarrow f_{V_1}(i) = f_{V_2}(i)$——加桶时条目可以从旧桶搬到新桶，不会从一个旧桶搬到另一个旧桶 |
| 分散性 | 跨全部 view，一个条目被映射到的不同桶的数量 $\mathrm{spread}(i)$ 较小 |
| 负载 | 跨全部 view，一个桶上出现的不同条目数量 $\mathrm{load}(b)$ 较小 |

**构造**：两张随机函数表——$r_B$ 把桶映射到单位区间 $[0,1)$，$r_I$ 把条目映射到同一区间；$f_V(i)$ 取 $V$ 中离 $r_I(i)$ 最近的桶。关键的一句在这里：**每个桶需要区间上不止一个点**——构造里每个桶用 $\gamma \log C$ 个点（$C$ 为桶总数的上界），等价于把每个桶复制 $\gamma \log C$ 份再各自随机落点。这就是虚拟节点，它在原始方案里就存在，不是后来的工程补丁。

**定理 4.1** 给出这个构造的四条性质：$f$ 单调；满足平衡性；分散性与负载都是 $O(t \log C)$（高概率），$t$ 是 view 规模下界系数的倒数。

**重映射界（定理 4.4）**：对单调且通用的 ranged hash 函数，view 从 $V_1$ 变到 $V_2$ 时，期望**不变**的条目比例是

$$\frac{|V_1 \cap V_2|}{|V_1 \cup V_2|}$$

代入扩容：$n-1$ 个桶加第 $n$ 个，$|V_1 \cap V_2| = n-1$、$|V_1 \cup V_2| = n$，不变比例 $(n-1)/n$——**期望只有 $1/n$ 的条目换手**，$K$ 个条目就是 $K/n$。缩容对称。定理还附带一条推论：靠放宽单调性换不来「更一致」的通用哈希函数。

**评估**：STOC 版的结果是分析式的，没有实验。协议在 $N$ 台缓存、每台平均 $\lambda$ 个请求的场景下，单台机器期望承载 $O(\lambda \log N \log \log N)$ 个请求，且以高概率不超过 $O(\lambda \log N \log \log N + \log N)$。实现与实测在 1999 年的期刊版本里（*Web Caching with Consistent Hashing*, Computer Networks）。

**与工程实现的差别**：Karger 1997 的哈希空间是单位区间 $[0,1)$，$2^{32}$ 环是实现层面的离散化——memcached 客户端（Ketama）把它压到 32 位整数环上，各客户端实现之间的取值与切分方式并不统一。

最后一个对照，把两种方案在扩容时的行为并列：

```
扩容前的环     ──[node0]──────────────[node1]──────────────[node2]──
                                        │
                              node3 插到这一段中间
                                        ▼
扩容后的环     ──[node0]──────────────[node1]────[node3]────[node2]──
                                        └─只有这一段换主─┘
                     期望换手量 = 1/N（N 为扩容后的节点数）
```

## 相关

- [[02-BASE 理论]] —— 功能分区与分片是 BASE 的第一层动作，哈希环是分片的实现手段之一
- [[03-一致性模型]] —— 名字里的「一致性」指**扩容缩容时映射的稳定性**，与一致性模型说的不是同一件事，别混

## 参考

- David Karger, Eric Lehman, Tom Leighton, Matthew Levine, Daniel Lewin, Rina Panigrahy. *Consistent Hashing and Random Trees: Distributed Caching Protocols for Relieving Hot Spots on the World Wide Web*. STOC 1997（第 29 届 ACM 计算理论研讨会）。

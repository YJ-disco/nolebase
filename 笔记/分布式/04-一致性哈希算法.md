...
前面讲的原理与实现都不是凭空来的。一致性哈希出自 Karger 等人在 STOC 1997 上的一篇论文（原题见「参考」）。
...
论文没有先给算法，而是先定义**一个方案该满足什么**。这四个指标可以直接用来判断任何一种分片方案：
...
论文的机制是：**每个节点（桶）映射到的是一系列伪随机的点，而不是环上的单个点**。这个设计承担两个作用：

1. **削弱数据倾斜**：一个节点的多个点把它的「份额」分散到环上各处，各节点分到的区间之和因此更接近均值 —— 这正是前面第五节「数据倾斜与虚拟节点」在数学上的依据；
2. **可调节比例**：**改变某个节点映射的点数，就能改变它承担的 key 比例**。这让异构集群（机器容量不同）可以按容量分配负载，而不必假设每台机器一样大。

所以「虚拟节点」不是后来的工程补丁，它在原始方案里就有 —— 本笔记第六节的实现正是这个机制。

> **可信度说明**：以上四个指标、$K/n$ 的界与「每个桶映射到多个点」这三点，我读到的是**二手转述（百科与博客）**，未直接核对论文原文。论文出处见下，引用时按此保留。
...
- 四个评判指标与重映射界的具体表述转述自：英文维基「Consistent hashing」条目、若干中文技术博客。**未核对论文原文。**

...
四个时刻的落点（$\delta$ 与 $\theta$ 的式子就是在这四个量之间做加减）：

```mermaid
sequenceDiagram
    participant B as B（客户端）
    participant A as A（时钟源）
    Note over B: 记下 T1 —— 请求离开本地
    B->>A: 请求
    Note over A: 记下 T2 —— 请求到达
    Note over A: 服务端内部处理（这一段就是 T3 − T2）
    Note over A: 记下 T3 —— 响应离开
    A-->>B: 响应
    Note over B: 记下 T4 —— 响应到达本地
```
...
sequenceDiagram
    ...
    Note over M: 记下 T1 —— Sync 离开的精确时刻
    M->>S: Sync
    Note over S: 记下 T2 —— Sync 到达本地
    M->>S: [Follow_Up：携带 T1]
    Note over S: 记下 T3 —— Delay_Req 离开本地
    S->>M: Delay_Req
    Note over M: 记下 T4 —— Delay_Req 到达的精确时刻
    M-->>S: Delay_Resp：携带 T4
    Note over S: 凑齐 T1–T4 ⇒ 算 meanPathDelay 与 offset
```
